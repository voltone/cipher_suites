defmodule CipherSuites do
  ...
  @high         [:'3des_ede_cbc', :aes_128_cbc, :aes_128_gcm,
                 :aes_256_cbc, :aes_256_gcm, :chacha20_poly1305]
  @medium       [:rc4_128, :idea_cbc]
  ...
  @key_size %{
    ...
    des40_cbc:          40,
    des_cbc:            56,
    '3des_ede_cbc':     112,  # OpenSSL treats 3DES as having an effective
  ...
  defp filter(tokens) do
    result = Enum.reduce(tokens, %{included: [], excluded: []}, fn
      "@STRENGTH", state   -> sort_by_strength(state)
      # "@SECLEVEL=" <> n    ->
      "!" <> cipher, state -> exclude(cipher, state)
      "-" <> cipher, state -> delete(cipher, state)
      "+" <> cipher, state -> move_to_end(cipher, state)
      cipher, state        -> append(cipher, state)
    end)
  ...
  defp move_to_end(cipher, state) do
    ...
    {last, first} = Enum.partition(state.included, &(&1 in ciphers))
  ...
  defp find("aRSA"),
    do: all_with_key_exchange([:rsa, :dhe_rsa, :srp_rsa, :ecdhe_rsa])
  ...
  defp find("kDHE"),
    do: all_with_key_exchange([:dhe_rsa, :dhe_dss, :dh_anon, :dhe_psk])
  defp find("kEDH"), do: find("kDHE")
  defp find("DH"),
    do: all_with_key_exchange([:dh_rsa, :dh_dss, :dhe_rsa, :dhe_dss,
                               :dh_anon, :dhe_psk])
  ...
  defp find("kEECDH"),
    do: all_with_key_exchange([:ecdhe_rsa, :ecdhe_ecdsa, :ecdh_anon])
  defp find("kECDHE"), do: find("kEECDH")
  defp find("ECDH"),
    do: all_with_key_exchange([:ecdhe_rsa, :ecdhe_ecdsa, :ecdh_rsa,
                               :ecdh_ecdsa, :ecdh_anon])
  ...
  defp find("SSLv3") do
    ...
    find_all(
      [:ecdhe_ecdsa, :ecdhe_rsa, :ecdh_ecdsa, :ecdh_rsa, :dhe_rsa, :dhe_dss,
       :rsa, :dh_anon, :ecdh_anon, :dhe_psk, :rsa_psk, :psk, :srp_anon,
       :srp_rsa, :srp_dss],
      [:aes_256_cbc, :"3des_ede_cbc", :aes_128_cbc, :des_cbc, :rc4_128,
       :null],
      [:sha, :md5],
      [:default_prf]
    )
  ...
  defp find("TLSv1.2") do
    ...
    find_any(
      [],
      [:chacha20_poly1305, :aes_256_gcm, :aes_128_gcm],
      [:null, :sha256, :sha384],
      [:sha256, :sha384]
    )
  end
  ...
  defp find("AES"),
    do: all_with_cipher([:aes_128_cbc, :aes_128_gcm, :aes_256_cbc,
                         :aes_256_gcm])
  ...
  defp find("kSRP"),
    do: all_with_key_exchange([:srp, :srp_rsa, :srp_dss, :srp_anon])
  ...
  defp find_all(key_exchanges, ciphers, hash_functions, prfs) do
    Enum.filter(all(), fn
      {key_exchange, cipher, hash} ->
        key_exchange in key_exchanges and
          cipher in ciphers and
          hash in hash_functions and
          :default_prf in prfs
  ...
  defp find_any(key_exchanges, ciphers, hash_functions, prfs) do
    Enum.filter(all(), fn
      {key_exchange, cipher, hash} ->
        key_exchange in key_exchanges or
          cipher in ciphers or
          hash in hash_functions or
          :default_prf in prfs
      {key_exchange, cipher, hash, prf} ->
        key_exchange in key_exchanges or
          cipher in ciphers or
